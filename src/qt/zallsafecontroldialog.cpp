...
// Copyright (c) 2017-2018 The AllSafe developers
// Distributed under the MIT/X11 software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include "zallsafecontroldialog.h"
#include "ui_zallsafecontroldialog.h"
...
std::list<std::string> ZAllSafeControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZAllSafeControlDialog::listMints;

ZAllSafeControlDialog::ZAllSafeControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZAllSafeControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZAllSafeControlDialog::~ZAllSafeControlDialog()
{
    delete ui;
}

void ZAllSafeControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZAllSafeControlDialog::updateList()
{
...
void ZAllSafeControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZAllSafeControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZAllSafe_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZAllSafeControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZAllSafeControlDialog::GetSelectedMints()
{
...
void ZAllSafeControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
