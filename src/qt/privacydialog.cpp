...
#include "zallsafecontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zSAFE ought to be enough for anybody." - Bill Gates, 2017
    ui->zSAFEpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzSAFESyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzSAFE->setEnabled(false);
        ui->pushButtonMintzSAFE->setToolTip(tr("zSAFE is currently disabled due to maintenance."));

        ui->pushButtonSpendzSAFE->setEnabled(false);
        ui->pushButtonSpendzSAFE->setToolTip(tr("zSAFE is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zSAFEpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzSAFE_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zSAFE is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zSAFE...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zSAFE in ") +
...
void PrivacyDialog::on_pushButtonSpendzSAFE_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zSAFE is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zSAFE
        sendzSAFE();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zSAFE
    sendzSAFE();
}

void PrivacyDialog::on_pushButtonZAllSafeControl_clicked()
{
    ZAllSafeControlDialog* zSAFEControl = new ZAllSafeControlDialog(this);
    zSAFEControl->setModel(walletModel);
    zSAFEControl->exec();
}

void PrivacyDialog::setZAllSafeControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzSAFESelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzSAFE()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid AllSafe Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zSAFEpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zSAFE is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " ALLSAFE </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zSAFE</b>";
    ...
    // use mints from zSAFE selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZAllSafeControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZAllSafeControlDialog::GetSelectedMints();
    }
    ...
    // Clear zallsafe selector in case it was used
    ZAllSafeControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zSAFE Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " AllSafe, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zSAFE Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zSAFE </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zSAFE "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zSAFE "));
    ui->labelzSAFEAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzSAFESyncStatus->setVisible(fShow);
}
