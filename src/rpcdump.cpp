...
Value importaddress(const Array& params, bool fHelp)
{
    ...
    if (address.IsValid()) {
        ...
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid AllSafe address or script");
    }
...
Value bip38encrypt(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "bip38encrypt \"allsafeaddress\"\n"
            "\nEncrypts a private key corresponding to 'allsafeaddress'.\n"
            "\nArguments:\n"
            "1. \"allsafeaddress\"   (string, required) The allsafe address for the private key (you must hold the key already)\n"
